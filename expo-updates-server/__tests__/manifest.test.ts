  ...
  test.each([
    [
      'ios',
      {
        ...
        url: `${process.env.NEXT_PUBLIC_HOSTNAME}/api/assets?asset=updates/test/1/bundles/ios-9d01842d6ee1224f7188971c5d397115.js&runtimeVersion=test&platform=ios`,
      },
    ],
    [
      'android',
      {
        ...
        fileExtension: '.bundle',
        contentType: 'application/javascript',
        url: `${process.env.NEXT_PUBLIC_HOSTNAME}/api/assets?asset=updates/test/1/bundles/android-82adadb1fb6e489d04ad95fd79670deb.js&runtimeVersion=test&platform=android`,
      },
    ],
  ])('returns latest %p manifest', async (platform, launchAssetExpectation) => {
    process.env.NEXT_PUBLIC_PRIVATE_KEY_PATH = 'updates/test/1/privatekey.pem';
    ...
    const firstAssetExpectation = {
      ...
      url: `${process.env.NEXT_PUBLIC_HOSTNAME}/api/assets?asset=updates/test/1/assets/4f1cb2cac2370cd5050681232e8575a8&runtimeVersion=test&platform=${platform}`,
    };
    const { req, res } = createMocks({
      method: 'GET',
      headers: {
